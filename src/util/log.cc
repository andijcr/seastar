...
void logger::failed_to_log(std::exception_ptr ex, fmt::string_view fmt_str, compat::source_location fmt_loc) noexcept
{
    try {
        lambda_log_writer writer([ex = std::move(ex), fmt_str, fmt_loc] (internal::log_buf::inserter_iterator it) {
            seastar::current_backtrace()
            it = fmt::format_to(it, "{}:{} @{}: failed to log message", fmt_loc.file_name(), fmt_loc.line(), fmt_loc.function_name());
            if (fmt_str.size() > 0) {
                it = fmt::format_to(it, ": fmt='{}'", fmt_str);
            }
